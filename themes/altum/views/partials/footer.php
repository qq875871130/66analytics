        ...
        <div class="col-12 col-sm-4 mb-3 mt-sm-0 d-flex flex-column">
            <a class="navbar-brand" href="<?= url() ?>">
                <?php if(settings()->logo != ''): ?>
                    <img src="<?= UPLOADS_FULL_URL . 'logo/' . settings()->logo ?>" class="footer-logo" alt="<?= language()->global->accessibility->logo_alt ?>" />
                <?php else: ?>
                    <?= settings()->main->title ?>
                <?php endif ?>
            </a>

            <div class="mb-3"><?= sprintf(language()->global->footer->copyright, date('Y'), settings()->main->title) ?></div>

            <?php if(settings()->email_notifications->contact && !empty(settings()->email_notifications->emails)): ?>
                <div class="mb-1">
                    <a href="<?= url('contact') ?>">
                        <i class="fa fa-fw fa-sm fa-envelope-open-text text-muted mr-1"></i> <?= language()->contact->menu ?>
                    </a>
                </div>
            <?php endif ?>
