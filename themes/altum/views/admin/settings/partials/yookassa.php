...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        <div class="form-group">
            <label for="is_enabled"><?= language()->admin_settings->yookassa->is_enabled ?></label>
            <select id="is_enabled" name="is_enabled" class="form-control form-control-lg">
                <option value="1" <?= settings()->yookassa->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->yes ?></option>
                <option value="0" <?= !settings()->yookassa->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->no ?></option>
            </select>
        </div>

        <div class="form-group">
            <label for="shop_id"><?= language()->admin_settings->yookassa->shop_id ?></label>
            <input id="shop_id" type="text" name="shop_id" class="form-control form-control-lg" value="<?= settings()->yookassa->shop_id ?>" />
        </div>

        <div class="form-group">
            <label for="secret_key"><?= language()->admin_settings->yookassa->secret_key ?></label>
