...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        <div class="form-group">
            <label for="is_enabled"><?= language()->admin_settings->paystack->is_enabled ?></label>
            <select id="is_enabled" name="is_enabled" class="form-control form-control-lg">
                <option value="1" <?= settings()->paystack->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->yes ?></option>
                <option value="0" <?= !settings()->paystack->is_enabled ? 'selected="selected"' : null ?>><?= language()->global->no ?></option>
            </select>
        </div>

        <div class="form-group">
            <label for="public_key"><?= language()->admin_settings->paystack->public_key ?></label>
            <input id="public_key" type="text" name="public_key" class="form-control form-control-lg" value="<?= settings()->paystack->public_key ?>" />
        </div>

        <div class="form-group">
            <label for="secret_key"><?= language()->admin_settings->paystack->secret_key ?></label>
